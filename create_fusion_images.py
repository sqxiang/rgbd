...
def process_comboimages(allfilesinfo, chandic, totchan, SAVEDIR):
	...
	for i in xrange(ftotsize):
		...
		outfile = SAVEDIR + '/' + os.path.splitext(fname)[0] + '_fus.jpg'
...
if __name__ == "__main__":
	...
	output_dir =  os.path.abspath('../images/train_fus')
        for eachFile in os.listdir(output_dir):
            if os.path.isfile(os.path.join(output_dir,eachFile)) and os.path.splitext(eachFile)[-1]=='.jpg':
                os.remove(os.path.join(output_dir,eachFile))
	chandic =[ [0,3],[3,6] ]
	num_channels = 6
	rgb_im_paths = sorted(glob.glob('../images/rgb_img1/new_human/*.jpg'))
	depth_im_paths = sorted(glob.glob('../images/depth_img1/new_human/*.jpg'))
	...
	oname = output_dir+'/trainlist.txt'
	file = open(oname, "w")
	for fnm in file_info :
	#	file.write(fnm[0].strip("/") +' %d\n'  % (fnm[1]))
		file.write(fnm[0].strip("/")+'\n')
	file.close()
	print "Wrote ", oname
